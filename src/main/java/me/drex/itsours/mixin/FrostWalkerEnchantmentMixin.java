...
public abstract class FrostWalkerEnchantmentMixin {
    ...
    private static boolean itsours$canFreezeWater(boolean original, LivingEntity entity, World world, BlockPos pos, int level) {
        ...
        return optional.map(claim -> claim.hasPermission(entity.getUuid(), PermissionManager.PLACE, Node.dummy(Registry.BLOCK, Blocks.FROSTED_ICE.getDefaultState().getBlock())) && original).orElse(original);
    }
